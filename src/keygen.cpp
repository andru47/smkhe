...
EvaluationKey Keygen::generateEvaluationKey() {
    ...
    vector<Polynomial<uint64_t>> secretSquared(params.getModulusLevels(), Polynomial<uint64_t>(params.getRingDegree()));
    for (int i = 0; i < params.getModulusLevels(); ++i) {
        for (int j = 0; j < params.getRingDegree(); ++j) {
            ...
            secretSquared[i].setCoeff(j, modMultiply(secretSquared[i].getCoeff(j), pModQ[i], params.getPrimes()[i]));
        }
    }
    ...
    for (int elementIndex = 0; elementIndex < params.getRingDegree(); ++elementIndex) {
        for (int i = 0; i < params.getSpecialPrimesNumber(); ++i) {
            ...
            bP[i].setCoeff(elementIndex,
                           modMultiply(bP[i].getCoeff(elementIndex), secretKey.getPolysP()[i].getCoeff(elementIndex),
        ...
        for (int i = 0; i < params.getModulusLevels(); ++i) {
            ...
            bQ[i].setCoeff(elementIndex,
                           modMultiply(bQ[i].getCoeff(elementIndex), secretKey.getPolysQ()[i].getCoeff(elementIndex),
                                       params.getPrimes()[i]));
            bQ[i].setCoeff(elementIndex, modAdd(bQ[i].getCoeff(elementIndex), secretSquared[i].getCoeff(elementIndex),
                                                params.getPrimes()[i]));
        }
    }
